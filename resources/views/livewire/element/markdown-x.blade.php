<div class="relative">
    <div x-data="markdown()" x-ref="markdownX" data-key="{{ $key }}" id="markdown-{{ $key }}"
        ...
        <div
            class="@if (isset($style['toolbar'])) {{ $style['toolbar'] }}@else{{ 'relative flex items-center justify-between w-full h-12 overflow-x-hidden bg-slate-50 sm:h-10' }} @endif">
            <div class="flex items-center h-12 sm:h-10">
                <div class="flex items-center h-full px-4 font-medium text-slate-600 cursor-pointer hover:bg-slate-100 dark-hover:bg-dark-950"
                    @click.prevent="section = 'write'"
                    x-bind:class="{ 'text-blue-500 border-b border-blue-500': section == 'write' }">
                ...
                <div wire:click="updateContentPreview()"
                    class="flex items-center h-full px-4 font-medium text-slate-600 cursor-pointer hover:bg-slate-100 dark-hover:bg-dark-950"
                    @click.prevent="section = 'preview'"
                    x-bind:class="{ 'text-blue-500 border-b border-blue-500': section == 'preview' }">
                    <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                        stroke-width="2" viewBox="0 0 24 24" class="w-4 h-4 mr-2">
                ...
                <div class="flex items-center h-full px-4 font-medium text-slate-600 cursor-pointer hover:bg-slate-100 dark-hover:bg-dark-950"
                    ...
                    x-bind:class="{ 'text-blue-500 border-b border-blue-500': section == 'help' }">
                    <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                        stroke-width="2" viewBox="0 0 24 24" class="w-4 h-4 mr-2">
                        <path
        ...
        <div class="relative z-40" x-show="section == 'write'">
            ...
            <div x-ref="editorModal" x-show="popup" x-ref="popup" x-on:keydown.escape="cancelModal(); popup=false"
                ...
                <div class="overflow-hidden border border-gray-200 rounded-lg">
                    ...
                    <div class="px-5 py-3 bg-slate-100 sm:flex sm:flex-row-reverse">
                        <span class="flex w-full rounded-md sm:ml-3 sm:w-auto">
                            <button id="modalClose" @click.prevent="cancelModal(); popup = false;" type="button"
                                class="inline-flex justify-center w-full px-4 py-2 mr-2 text-base font-medium leading-6 text-slate-700 transition duration-150 ease-in-out bg-white border border-gray-300 rounded-md shadow-sm hover:text-slate-500 focus:outline-none focus:border-blue-300 focus:shadow-outline sm:text-sm sm:leading-5">Cancel</button>
                            <button type="button" x-ref="modalExecute" id="modal-execute-{{ $key }}"
                                ...
                                class="inline-flex justify-center w-full px-4 py-2 text-base font-medium leading-6 text-white transition duration-150 ease-in-out bg-blue-500 border-blue-500 rounded-md shadow-sm text-whit hover:border-blue-600 hover:bg-blue-600 focus:outline-none focus:border-blue-300 focus:shadow-outline sm:text-sm sm:leading-5">Insert</button>
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div
                ...
                <div x-ref="drop" x-show="dropFiles" x-on:dragleave="$event.preventDefault(); dropFiles=false"
                    ...
                    <div
                        class="flex flex-col items-center justify-center w-40 h-32 text-xs text-slate-400 bg-white border-0 border-gray-200 border-dashed rounded-lg">
        ...
        <div x-show="section == 'preview'" wire:target="updateContentPreview"
            class="@if (isset($style['preview'])) {{ $style['preview'] }}@else{{ 'h-full bg-white min-h-screen relative z-30 px-5 pt-5 prose md:prose-xl lg:prose-2xl max-w-none' }} @endif"
            x-cloak>
            {!! $contentPreview !!}
        </div>
        ...
        <div x-show="section == 'help'"
            class="@if (isset($style['help'])) {{ $style['help'] }}@else{{ 'h-full bg-white min-h-screen px-5 pt-10 relative z-30 prose bg-white dark:bg-dark-900 md:prose-xl lg:prose-lg max-w-none' }} @endif"
            x-cloak>
            ...
            <p><span class="mr-2">Inline Code</span><br><span
                    class="p-1 font-mono text-blue-500 bg-blue-100 rounded-md">`backtick`</span><span
                    class="block mt-2 mr-2">Code Block</span><span
                    class="block p-1 font-mono text-blue-600 bg-blue-100">```<br>Three back ticks and then enter your
            ...
            <p>You can add <a href="#_">links</a> by adding text inside of <span
                    class="p-1 font-mono text-blue-600 bg-blue-100">[]</span> and the link inside of <span
                    class="p-1 font-mono text-blue-600 bg-blue-100">()</span>, like so:</p>
            ...
            <p>You can add images by selecting the image icon, which will upload and add an image to the editor, or you
                can manually add the image by adding an exclamation <span
                    class="p-1 font-mono text-blue-600 bg-blue-100">!</span>, followed by the alt text inside of <span
                    class="p-1 font-mono text-blue-600 bg-blue-100">[]</span>, and the image URL inside of <span
                    class="p-1 font-mono text-blue-600 bg-blue-100">()</span>, like so:</p>
            ...
            <p>To add a divider you can add three dashes or three asterisks:<br><span
                    class="block p-1 pl-5 font-mono text-blue-600 bg-blue-100">--- or ***</span>
            </p>
            <hr>
            ...
            @if (in_array('giphy', config('markdownx.dropdown_items')))
                ...
                <p>You can easily embed animated GIFS with the following syntax:<br><span
                        class="p-1 font-mono text-blue-600 bg-blue-100">{% giphy https://giphy.com/embed/giphy_id %}</span>
                </p>
                <hr>
            @endif
            ...
            @if (in_array('codepen', config('markdownx.dropdown_items')))
                ...
                <p>You can also embed a codepen by writing the following:<br><span
                        class="p-1 font-mono text-blue-600 bg-blue-100">{% codepen https://codepen.io/your/pen/url %}</span>
                </p>
            ...
            @if (in_array('codesandbox', config('markdownx.dropdown_items')))
                ...
                <p>You can also embed CodeSandbox by writing the following:<br><span
                        class="p-1 font-mono text-blue-600 bg-blue-100">{% codesandbox YOUR_CODESANDBOX_EMBED_URL %}</span>
                </p>
                <hr>
            @endif
            ...
            @if (in_array('gists', config('markdownx.dropdown_items')))
                ...
                <p>You can also embed a Gists by writing the following:<br><span
                        class="p-1 font-mono text-blue-600 bg-blue-100">{% gist GIST_ID_HERE %}</span></p>
                <hr>
            @endif
            ...
            @if (in_array('youtube', config('markdownx.dropdown_items')))
                ...
                <p>You can also embed a YouTube video by writing the following:<br><span
                        class="p-1 font-mono text-blue-600 bg-blue-100">{% youtube VIDEO_ID_HERE %}</span></p>
                <hr>
            @endif
    ...
    <script>
        ...
        window.markdown = function() {
            return {
                ...
                createDropDownElement() {
                    ...
                    marker.className =
                        'absolute z-50 block w-auto overflow-hidden transition-all duration-100 ease-in-out transform scale-95 translate-x-0 bg-white border border-gray-200 rounded-md shadow-xl translate-y-7 whitespace-wrap';
                    return marker;
                },
                isNormalInteger(str) {
                    str = str.trim();
                ...
                setModalHTML(suggestion) {
                    ...
                    modalSuggestionHTML["link"] =
                        `<label for="text" class="block mb-1.5 text-sm font-medium leading-5 text-slate-700 sm:mt-px">
                            Link Text
                        </label>
                        ...
                        <label for="text" class="block mb-1.5 text-sm font-medium leading-5 text-slate-700 sm:mt-px sm:pt-2">
                            Link URL
                        </label>
                    ...
                    modalSuggestionHTML["giphy"] = `<label for="text" class="block mb-1.5 text-sm font-medium leading-5 text-slate-700 sm:mt-px">
                        ...
                        <div id="giphy-items" class="grid w-full h-64 grid-cols-3 gap-1 p-1 mt-2 overflow-y-scroll border border-gray-200 rounded-lg bg-slate-50 grid-cols">
                ...
                getSuggestionsHTML() {
                    let suggestionHTML = `<span class="relative flex items-center w-full p-3 rounded-lg">
                                            ...
                                            <span class="flex flex-col mx-3">
                                                <h4 class="text-xs font-medium mt-0.5 leading-none text-slate-700">[[ title ]]</h4>
                                                <p class="text-xs mt-0.5 text-slate-400">[[ description ]]</p>
                                            </span>
                                        </span>`;
                ...
                filterList() {
                    ...
                    if (!filteredSuggestions.length) {
                        ...
                        suggestionHTML['none'] =
                            '<div class="px-4 py-2 text-sm font-medium text-slate-500">No suggestions found.</div>';
                    }
        ...
        window.addEventListener('markdown-x-giphy-results', event => {
            ...
            for (var i = 0; i < giphyResults.length; i++) {
                giphyContents +=
                    `<img src="${giphyResults[i]['image']}" onclick="addAnimatedGif('{% giphy ${giphyResults[i]['embed']} %}', '${event.detail.key}')" class="w-full h-auto border-2 border-gray-200 rounded cursor-pointer hover:border-blue-500" />`;
                if (i % 10 == 0 && i != 0) {
