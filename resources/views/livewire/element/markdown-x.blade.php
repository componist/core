<div class="relative">
    <div x-data="markdown()" x-ref="markdownX" data-key="{{ $key }}" id="markdown-{{ $key }}"
        ...
        <div
            class="@if (isset($style['toolbar'])) {{ $style['toolbar'] }}@else{{ 'relative flex items-center justify-between w-full h-12 overflow-x-hidden bg-gray-50 sm:h-10' }} @endif">
            <div class="flex items-center h-12 sm:h-10">
                <div class="flex items-center h-full px-4 font-medium text-gray-600 cursor-pointer hover:bg-gray-100 dark-hover:bg-dark-950"
                ...
                <div wire:click="updateContentPreview()"
                    class="flex items-center h-full px-4 font-medium text-gray-600 cursor-pointer hover:bg-gray-100 dark-hover:bg-dark-950"
            ...
            <div
                class="relative flex items-center h-full px-4 font-medium text-gray-600 cursor-pointer hover:bg-gray-100 dark-hover:bg-dark-950">
        ...
        <div class="relative z-40" x-show="section == 'write'">
            ...
            <div wire:ignore x-show="debug" @click.prevent="$refs.editor.focus()"
                :class="{ 'w-full h-full bg-red-100 bg-opacity-50': debuggerOpen, 'w-0 h-auto': !debuggerOpen }"
                ...
                <div x-ref="debugButton" class="relative opacity-0">
                    <div @click.prevent="debuggerOpen=!debuggerOpen"
                        :class="{
                            'text-gray-400 bg-gray-50 hover:text-gray-500': !
                                debuggerOpen,
            ...
            <div
                ...
                <div wire:ignore x-ref="placeholder" @click.prevent="$refs.editor.focus()"
                    ...
                    class="absolute z-20 text-gray-400 transition-opacity duration-200 ease-out" x-cloak>Type '/' for
                    commands </div>
                <textarea x-ref="editor" id="editor-{{ $key }}" data-key="{{ $key }}"
                    class="editors @if (isset($style['textarea'])) {{ $style['textarea'] }}@else{{ 'w-full h-full mx-auto min-h-screen px-5 md:px-1 pt-5 font-mono leading-loose tracking-tighter border-0 outline-none focus:outline-none sm:x-0 text-lg text-gray-600' }} @endif"
                ...
                <div x-ref="drop" x-show="dropFiles" x-on:dragleave="$event.preventDefault(); dropFiles=false"
                    ...
                    class="absolute inset-0 flex items-center justify-center w-full h-full bg-blue-100 bg-opacity-20"
                    x-cloak>
                    <div
                        class="flex flex-col items-center justify-center w-40 h-32 text-xs text-gray-400 bg-white border-0 border-gray-200 border-dashed rounded-lg">
            ...
            return {
                ...
                setModalHTML(suggestion) {
                    ...
                    modalSuggestionHTML["link"] =
                        `<label for="text" class="block mb-1.5 text-sm font-medium leading-5 text-gray-700 sm:mt-px">
                            Link Text
                        </label>
                ...
                filterList() {
                    ...
                    if (!filteredSuggestions.length) {
                        ...
                        suggestionHTML['none'] =
                            '<div class="px-4 py-2 text-sm font-medium text-gray-500">No suggestions found.</div>';
                    }
