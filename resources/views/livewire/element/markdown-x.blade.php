<div class="relative">
    <div x-data="markdown()" x-ref="markdownX" data-key="{{ $key }}" id="markdown-{{ $key }}" class="relative w-full"
        ...
        <div
            class="@if (isset($style['toolbar'])) {{ $style['toolbar'] }}@else{{ 'relative flex items-center justify-between w-full h-12 overflow-x-hidden bg-slate-50 dark:bg-slate-900 sm:h-10' }} @endif">
            <div class="flex items-center h-12 sm:h-10">
                <div class="flex items-center h-full px-4 font-medium text-slate-600 dark:text-slate-300 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800"
                    @click.prevent="section = 'write'"
                    x-bind:class="{ 'text-teal-500 border-b border-teal-500': section == 'write' }">
                    <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                        stroke-width="2" viewBox="0 0 24 24" class="w-4 h-4 mr-2">
                        <path
                ...
                <div wire:click="updateContentPreview()"
                    class="flex items-center h-full px-4 font-medium text-slate-600 dark:text-slate-300 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800"
                    @click.prevent="section = 'preview'"
                    x-bind:class="{ 'text-teal-500 border-b border-teal-500': section == 'preview' }">
                    <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                        stroke-width="2" viewBox="0 0 24 24" class="w-4 h-4 mr-2">
                ...
                <div class="flex items-center h-full px-4 font-medium text-slate-600 dark:text-slate-300 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800"
                    @click.prevent="section = 'help'"
                    x-bind:class="{ 'text-teal-500 border-b border-teal-500': section == 'help' }">
                    <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                        stroke-width="2" viewBox="0 0 24 24" class="w-4 h-4 mr-2">
                        <path
            ...
            <div
                class="relative flex items-center h-full px-4 font-medium text-slate-600 dark:text-slate-300 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800">
        ...
        <div class="relative z-40" x-show="section == 'write'">
            <div x-ref="error" id="error-{{ $key }}"
                class="absolute top-0 z-40 hidden w-full py-2 text-sm text-center text-red-400 bg-red-50 dark:bg-red-950/40 dark:text-red-300"></div>
            <div x-ref="editorModal" x-show="popup" x-ref="popup" x-on:keydown.escape="cancelModal(); popup=false"
                ...
                <div
                    class="absolute left-0 w-4 h-4 -mt-2 ml-3.5 transform rotate-45 bg-white dark:bg-slate-900 border-t border-l border-gray-200 dark:border-slate-700 rounded-tl-sm">
                </div>
                <div class="overflow-hidden border border-gray-200 dark:border-slate-700 rounded-lg">
                    ...
                    <div class="px-5 py-3 bg-slate-100 dark:bg-slate-800 sm:flex sm:flex-row-reverse">
                        <span class="flex w-full rounded-md sm:ml-3 sm:w-auto">
                            <button id="modalClose" @click.prevent="cancelModal(); popup = false;" type="button"
                                class="inline-flex justify-center w-full px-4 py-2 mr-2 text-base font-medium leading-6 text-slate-700 dark:text-slate-200 transition duration-150 ease-in-out bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded-md shadow-sm hover:text-slate-500 dark:hover:text-slate-300 focus:outline-none focus:border-blue-300 focus:shadow-outline sm:text-sm sm:leading-5">Abbrechen</button>
                            <button type="button" x-ref="modalExecute" id="modal-execute-{{ $key }}"
                                @click.prevent="executeAssociatedFunction()" data-suggestion=""
                                @click.prevent="popup = false"
                                class="inline-flex justify-center w-full px-4 py-2 text-base font-medium leading-6 text-white transition duration-150 ease-in-out bg-teal-500 border-teal-500 rounded-md shadow-sm hover:border-teal-600 hover:bg-teal-600 focus:outline-none focus:border-teal-300 focus:shadow-outline sm:text-sm sm:leading-5">Einfügen</button>
                        </span>
                    </div>
                </div>
            </div>
            <div id="dropdown-{{ $key }}" x-ref="dropdown" @click.prevent="clickItem($event)" wire:ignore
                class="relative z-40"></div>
            <div wire:ignore x-show="debug" @click.prevent="$refs.editor.focus()"
                :class="{ 'w-full h-full bg-red-100/50': debuggerOpen, 'w-0 h-auto': !debuggerOpen }"
                ...
                <div x-ref="debugButton" class="relative opacity-0">
                    <div @click.prevent="debuggerOpen=!debuggerOpen" :class="{
                            'text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-800 hover:text-slate-500 dark:hover:text-slate-400': !
                                debuggerOpen,
                            'text-red-400 hover:text-red-500 bg-red-50': debuggerOpen
                        }"
                        class="absolute top-0 flex items-center justify-center rounded-sm cursor-pointer -ml-9 group w-9 h-9">
                        <svg x-show="debuggerOpen" class="w-5 h-5 transform rotate-90 stroke-current"
                            xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" fill="none"
                        ...
                        <svg x-show="!debuggerOpen" x-transition:enter-start="rotate-90"
                            x-transition:enter-end="rotate-0"
                            class="w-5 h-5 transition-all duration-500 ease-out transform stroke-current group-hover:rotate-90"
                            xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" fill="none"
                            stroke-linecap="round" stroke-linejoin="round">
                            <path d="M0 0h24v24H0z" stroke="none" />
                            <path
                ...
                <div x-show="debuggerOpen" class="fixed bottom-0 right-0 bg-red-50 px-3 py-2 dark:bg-red-950/50">
                    <div class="flex text-sm text-red-500 dark:text-red-300">
            ...
            <div class="relative z-30 overflow-hidden @if (isset($style['height'])) {{ $style['height'] }} @endif">
                ...
                <div wire:ignore x-ref="placeholder" @click.prevent="$refs.editor.focus()" id="placeholder-{{ $key }}"
                    x-show="placeholder" class="absolute z-20 text-slate-400 dark:text-slate-500 transition-opacity duration-200 ease-out"
                    x-cloak>'/' für Befehle tippen</div>
                <textarea x-ref="editor" id="editor-{{ $key }}" data-key="{{ $key }}"
                    class="editors @if (isset($style['textarea'])) {{ $style['textarea'] }}@else{{ 'w-full h-full mx-auto min-h-screen px-5 md:px-1 pt-5 font-mono leading-loose tracking-tighter border-0 outline-none focus:outline-none sm:x-0 text-lg text-slate-600 dark:text-slate-300 dark:bg-slate-950' }} @endif"
                    placeholder="" data-loaded="false" name="{{ $name }}"
                    x-on:dragenter="$event.preventDefault(); dropFiles=true" wire:model.lazy="content"
                    x-on:blur="@this.call('update', { content: $event.target.value });" x-on:focus="editorEvent($event)"
                    x-on:keypress="editorEvent($event)" x-on:keydown="getCursorXY(); editorEvent($event)"
                    x-on:keyup="getCursorXY(); editorEvent($event)" x-on:click="editorEvent($event)" x-cloak>
                    </textarea>
                <div x-ref="drop" x-show="dropFiles" x-on:dragleave="$event.preventDefault(); dropFiles=false"
                    x-on:dragover="$event.preventDefault();" x-on:drop="$event.preventDefault(); droppingFile($event)"
                    class="absolute inset-0 flex items-center justify-center w-full h-full bg-blue-100/20" x-cloak>
                    <div
                        class="flex flex-col items-center justify-center w-40 h-32 text-xs text-slate-400 dark:text-slate-500 bg-white dark:bg-slate-800 border-0 border-gray-200 dark:border-slate-600 border-dashed rounded-lg">
                        <svg class="w-12 h-auto mb-3 fill-current" viewBox="0 0 98 97"
                            xmlns="http://www.w3.org/2000/svg">
                            <g fill-rule="nonzero">
                                <path
                                    d="M63.48.16H2.76a2 2 0 00-2 2v60.73a2 2 0 002 2h23.59a2 2 0 100-4H4.78V4.22h56.66v21.57a2 2 0 104 0V2.17a2 2 0 00-2-2M95.35 89.08A1.91 1.91 0 0093.44 91v1.9h-1.9a1.91 1.91 0 000 3.81h3.81a1.91 1.91 0 001.9-1.9V91a1.91 1.91 0 00-1.9-1.9M77.55 92.88h-6.32a1.91 1.91 0 000 3.81h6.32a1.91 1.91 0 100-3.81M57.24 92.88h-6.32a1.91 1.91 0 100 3.81h6.32a1.91 1.91 0 000-3.81M38.19 92.88h-1.91V91a1.9 1.9 0 00-3.8 0v3.81a1.91 1.91 0 001.9 1.9h3.81a1.91 1.91 0 000-3.81" />
                                <path
                                    d="M34.38 58.58a1.91 1.91 0 001.9-1.9v-6.32a1.9 1.9 0 00-3.8 0v6.32a1.89 1.89 0 001.9 1.9M32.48 77a1.9 1.9 0 003.8 0v-6.33a1.9 1.9 0 00-3.8 0V77zM38.19 31.92h-3.81a1.9 1.9 0 00-1.9 1.9v3.8a1.9 1.9 0 003.8 0v-1.9h1.91a1.91 1.91 0 001.9-1.9 1.89 1.89 0 00-1.9-1.9M57.24 31.92h-6.32a1.9 1.9 0 000 3.8h6.32a1.91 1.91 0 001.9-1.9 1.89 1.89 0 00-1.9-1.9M77.55 31.92h-6.32a1.9 1.9 0 000 3.8h6.32a1.9 1.9 0 000-3.8M95.35 31.92h-3.81a1.9 1.9 0 000 3.8h1.9v1.9a1.91 1.91 0 103.81 0v-3.8a1.91 1.91 0 00-1.9-1.9M95.35 68.74a1.9 1.9 0 00-1.91 1.9V77a1.91 1.91 0 103.81 0v-6.33a1.93 1.93 0 00-1.9-1.93M95.35 48.42a1.91 1.91 0 00-1.91 1.91v6.35a1.91 1.91 0 003.81 0v-6.32a1.93 1.93 0 00-1.9-1.94M81.41 62.81a1.29 1.29 0 01-.89 1.19l-10.33 3.65a3.26 3.26 0 00-2 2L64.57 80a1.29 1.29 0 01-1.23.89 1.34 1.34 0 01-1.26-.89L51.45 52.65a1.28 1.28 0 01.31-1.43 1.3 1.3 0 01.92-.39c.17.001.338.028.5.08l27.39 10.64a1.2 1.2 0 01.84 1.26" />
                            </g>
                        </svg>
        ...
        <div x-show="section == 'preview'" wire:target="updateContentPreview"
            class="@if (isset($style['preview'])) {{ $style['preview'] }}@else{{ 'h-full bg-white dark:bg-slate-900 min-h-screen relative z-30 px-5 pt-5 prose dark:prose-invert md:prose-xl lg:prose-2xl max-w-none' }} @endif"
            x-cloak>
            {!! $contentPreview !!}
        </div>
        ...
        <div x-show="section == 'help'"
            class="@if (isset($style['help'])) {{ $style['help'] }}@else{{ 'h-full bg-white dark:bg-slate-900 min-h-screen px-5 pt-10 relative z-30 prose dark:prose-invert md:prose-xl lg:prose-lg max-w-none' }} @endif"
            x-cloak>

            <h2>Markdown Basics</h2>
            <p>Below you will find some common used markdown syntax. For a deeper dive in Markdown check out this <a
                    href="https://github.com/adam-p/markdown-here/wiki/Markdown-Here-Cheatsheet" target="_blank">Cheat
                    Sheet</a></p>
            <hr>

            <h3>Bold & Italic</h3>
            <p><span class="mr-2 italic">Italics</span> <span class="p-1">*asterisks*</span><br><span
                    class="mr-2 font-bold">Bold</span> <span class="p-1">**double asterisks**</span></p>
            <hr>

            <h3>Code</h3>
            <p><span class="mr-2">Inline Code</span><br><span
                    class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40 rounded-md">`backtick`</span><span
                    class="block mt-2 mr-2">Code Block</span><span
                    class="block p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">```<br>Three back ticks and then enter your
                    code blocks here.<br>```</span></p>
            <hr>
            ...
            <h3>Links</h3>
            <p>You can add <a href="#_">links</a> by adding text inside of <span
                    class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">[]</span> and the link inside of <span
                    class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">()</span>, like so:</p>
            <div class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">[link_text](https://google.com)</div>
            <hr>

            <h3>Lists</h3>
            <p>To add a numbered list you can simply start with a number and a <span
                    class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">.</span>, like so:<br><span
                    class="block p-1 pl-5 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40"> 1. The first item in my list</span></p>
            <p>For an unordered list, you can add a dash <span class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">-</span>,
                like so:<br><span class="block p-1 pl-5 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40"> - The start of my
                    list</span></p>
            <hr>

            <h3>Images</h3>
            <p>You can add images by selecting the image icon, which will upload and add an image to the editor, or you
                can manually add the image by adding an exclamation <span
                    class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">!</span>, followed by the alt text inside of <span
                    class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">[]</span>, and the image URL inside of <span
                    class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">()</span>, like so:</p>
            <div class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">![alt text for image](url_to_image.png)</div>
            <hr>

            <h3>Dividers</h3>
            <p>To add a divider you can add three dashes or three asterisks:<br><span
                    class="block p-1 pl-5 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">--- or ***</span>
            </p>
            <hr>

            @if (in_array('giphy', config('markdownx.dropdown_items')))
                <h3>Embedding GIFs via Giphy</h3>
                <p>You can easily embed animated GIFS with the following syntax:<br><span
                        class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">{% giphy https://giphy.com/embed/giphy_id %}</span>
                </p>
                <hr>
            @endif
            ...
            @if (in_array('codepen', config('markdownx.dropdown_items')))
                ...
                <p>You can also embed a codepen by writing the following:<br><span
                        class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">{% codepen https://codepen.io/your/pen/url %}</span>
                </p>
                <p>You may also choose the default tabs you wish to show your pen by writing the <span
                        class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">default-tab</span> like so: (default is result)
                </p>
                <p><span class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">{% codepen https://codepen.io/your/pen/url
            ...
            @if (in_array('codesandbox', config('markdownx.dropdown_items')))
                ...
                <p>You can also embed CodeSandbox by writing the following:<br><span
                        class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">{% codesandbox YOUR_CODESANDBOX_EMBED_URL %}</span>
                </p>
                <hr>
            @endif
            ...
            @if (in_array('gists', config('markdownx.dropdown_items')))
                ...
                <p>You can also embed a Gists by writing the following:<br><span
                        class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">{% gist GIST_ID_HERE %}</span></p>
                <hr>
            @endif

            @if (in_array('youtube', config('markdownx.dropdown_items')))
                <h3>Embedding YouTube Videos</h3>
                <p>You can also embed a YouTube video by writing the following:<br><span
                        class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">{% youtube VIDEO_ID_HERE %}</span></p>
                <hr>
            @endif
            ...
            @if (in_array('buy_me_a_coffee', config('markdownx.dropdown_items')))
                ...
                <p>You can also embed your "Buy me a coffee" button by writing the following:<br><span
                        class="p-1 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40">{% buymeacoffee BUY_ME_A_COFFEE_USERNAME_HERE
                        %}</span>
                </p>
                <hr>
            @endif
    ...
    <script>
        ...
        const suggestionActiveClasses = 'bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white suggestion-active';
        const suggestionItemClasses = 'cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800';
        ...
        window.markdown = function () {
            return {
                ...
                loop() {
                    ...
                    if (this.currentCaretPos.start != newCurrentCaretPos.start || this.currentCaretPos.end !=
                        newCurrentCaretPos.end) {
                ...
                upload(evt, key) {
                    ...
                    if (file.size > (parseInt('{{ config('markdownx.image.max_file_size') }}') * 1000)) {
                        showErrorMessage(this.$refs.error, 'File size too large, please upload an image smaller than ' +
                            Math.round(parseInt('{{ config('markdownx.image.max_file_size') }}') / 1000) + 'MB.')
                        return null;
                    } else {
                        // we have a file
                        if (evt.target.value.length) {
                            ...
                            reader.onloadend = () => {
                                window.livewire.emit('markdown-x-image-upload', {
                                    image: reader.result,
                                    name: file.name,
                                    key: key,
                ...
                createDropDownElement() {
                    ...
                    marker.className =
                        'absolute z-50 block w-auto overflow-hidden transition-all duration-100 ease-in-out transform scale-95 translate-x-0 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-md shadow-xl translate-y-7 whitespace-wrap';
                    return marker;
                },
                isNormalInteger(str) {
                    str = str.trim();
                ...
                setModalHTML(suggestion) {
                    ...
                    modalSuggestionHTML["link"] =
                        `<label for="text" class="block mb-1.5 text-sm font-medium leading-5 text-slate-700 dark:text-slate-200 sm:mt-px">
                            Link Text
                        </label>
                        <input type="text" @keydown="submitModal(event)" id="editor-link-text" class="mb-2 block w-full rounded-md border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none transition duration-150 ease-in-out focus:border-teal-500 focus:ring-2 focus:ring-teal-500/30 dark:border-slate-600 dark:bg-slate-800 dark:text-white sm:text-sm sm:leading-5" placeholder="Text">
                        <label for="text" class="block mb-1.5 text-sm font-medium leading-5 text-slate-700 dark:text-slate-200 sm:mt-px sm:pt-2">
                            Link URL
                        </label>
                        <input type="text" @keydown="submitModal(event)" id="editor-link-url" class="block w-full rounded-md border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none transition duration-150 ease-in-out focus:border-teal-500 focus:ring-2 focus:ring-teal-500/30 dark:border-slate-600 dark:bg-slate-800 dark:text-white sm:text-sm sm:leading-5" placeholder="URL">`;
                    modalSuggestionHTML["code"] =
                        `<label for="text" class="block mb-1.5 text-sm font-medium leading-5 text-slate-700 dark:text-slate-200 sm:mt-px">
                            Code
                        </label>
                        <div id="editor-code" class="mb-2 block w-full rounded-md border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none transition duration-150 ease-in-out focus:border-teal-500 focus:ring-2 focus:ring-teal-500/30 dark:border-slate-600 dark:bg-slate-800 dark:text-white sm:text-sm sm:leading-5"></div>`;
                    modalSuggestionHTML["giphy"] = `<label for="text" class="block mb-1.5 text-sm font-medium leading-5 text-slate-700 dark:text-slate-200 sm:mt-px">
                            Insert an animated GIF
                        </label>
                        <input type="text" @keydown="searchGIFModal(event)" id="editor-giphy-search" class="block w-full rounded-md border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none transition duration-150 ease-in-out focus:border-teal-500 focus:ring-2 focus:ring-teal-500/30 dark:border-slate-600 dark:bg-slate-800 dark:text-white sm:text-sm sm:leading-5" placeholder="Search for a GIF">
                        <div id="giphy-items" class="grid w-full h-64 grid-cols-3 gap-1 p-1 mt-2 overflow-y-scroll border border-gray-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800 grid-cols">
                            <div class="absolute inset-0 flex items-center justify-center w-full h-full">
                                <svg class="w-5 h-5 text-slate-400 dark:text-slate-500 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                </svg>
                            </div>
                        </div>`;
                    modalSuggestionHTML["codepen"] =
                        `<label for="text" class="block mb-1.5 text-sm font-medium leading-5 text-slate-700 dark:text-slate-200 sm:mt-px">
                            Codepen URL
                        </label>
                        <input type="text" @keydown="submitModal(event)" id="editor-codepen-url" class="mb-2 block w-full rounded-md border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none transition duration-150 ease-in-out focus:border-teal-500 focus:ring-2 focus:ring-teal-500/30 dark:border-slate-600 dark:bg-slate-800 dark:text-white sm:text-sm sm:leading-5" placeholder="Codepen URL">`;
                    modalSuggestionHTML["codesandbox"] =
                        `<label for="text" class="block mb-1.5 text-sm font-medium leading-5 text-slate-700 dark:text-slate-200 sm:mt-px">
                            Codesandbox URL
                        </label>
                        <input type="text" @keydown="submitModal(event)" id="editor-codesandbox-url" class="mb-2 block w-full rounded-md border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none transition duration-150 ease-in-out focus:border-teal-500 focus:ring-2 focus:ring-teal-500/30 dark:border-slate-600 dark:bg-slate-800 dark:text-white sm:text-sm sm:leading-5" placeholder="Codesandbox URL">`;
                    modalSuggestionHTML["youtube"] =
                        `<label for="text" class="block mb-1.5 text-sm font-medium leading-5 text-slate-700 dark:text-slate-200 sm:mt-px">
                            Youtube URL
                        </label>
                        <input type="text" @keydown="submitModal(event)" id="editor-youtube-url" class="mb-2 block w-full rounded-md border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none transition duration-150 ease-in-out focus:border-teal-500 focus:ring-2 focus:ring-teal-500/30 dark:border-slate-600 dark:bg-slate-800 dark:text-white sm:text-sm sm:leading-5" placeholder="Youtube URL">`;
                    modalSuggestionHTML["buy_me_a_coffee"] =
                        `<label for="text" class="block mb-1.5 text-sm font-medium leading-5 text-slate-700 dark:text-slate-200 sm:mt-px">
                            Buy Me A Coffee Username
                        </label>
                        <input type="text" @keydown="submitModal(event)" id="editor-buy-me-a-coffee-username" class="mb-2 block w-full rounded-md border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none transition duration-150 ease-in-out focus:border-teal-500 focus:ring-2 focus:ring-teal-500/30 dark:border-slate-600 dark:bg-slate-800 dark:text-white sm:text-sm sm:leading-5" placeholder="Username">`;
                ...
                getSuggestionsHTML() {
                    let suggestionHTML = `<span class="relative flex items-center w-full p-3 rounded-lg">
                                            ...
                                            <span class="flex flex-col mx-3">
                                                <h4 class="text-xs font-medium mt-0.5 leading-none text-slate-700 dark:text-slate-200">[[ title ]]</h4>
                                                <p class="text-xs mt-0.5 text-slate-400 dark:text-slate-500">[[ description ]]</p>
                                            </span>
                                        </span>`;
                ...
                searchGIFModal(event) {
                    if (event.keyCode === 13) {
                        ...
                        if (event.target.value.trim() != "") {
                            window.livewire.emit('markdown-x-giphy-search', {
                ...
                scrollParentToChild(parent, child) {
                    ...
                    var isViewable = (childRect.top >= parentRect.top) && (childRect.bottom <= parentRect.top +
                        parentViewableArea.height);
                ...
                showModalPop(callback) {
                    ...
                    let newLeft = Math.min(x - editor.scrollLeft, editor.offsetLeft + editor.offsetWidth - parseInt(
                        paddingRight, 10));
                    let newTop = Math.min(y - editor.scrollTop + parseInt(lineHeight), editor.offsetTop + editor
                ...
                replaceSuggestionText(content) {
                    ...
                    this.$refs.editor.setCaretPosition(this.editStart + content.length, this.editStart + content
                        .length);
                },
                clearSuggestionText() {
                    let editor = this.$refs.editor;
                ...
                handleSpaceFunctionality() {
                    if (this.lineIsStartOfNumberedList(this.getCurrentLine()) || this.lineIsStartOfBulletedList(this
                        ...
                        this.$refs.editor.setCaretPosition(this.currentCaretPos.start + 1, this.currentCaretPos.end +
                            1);
                    }
                },
                ...
                filterList() {
                    ...
                    if (!filteredSuggestions.length) {
                        ...
                        suggestionHTML['none'] =
                            '<div class="px-4 py-2 text-sm font-medium text-slate-500 dark:text-slate-400">No suggestions found.</div>';
                    }
        ...
        window.addEventListener('markdown-x-giphy-results', event => {
            ...
            for (var i = 0; i < giphyResults.length; i++) {
                giphyContents +=
                    `<img src="${giphyResults[i]['image']}" onclick="addAnimatedGif('{% giphy ${giphyResults[i]['embed']} %}', '${event.detail.key}')" class="w-full h-auto border-2 border-gray-200 dark:border-slate-700 rounded cursor-pointer hover:border-teal-500 dark:hover:border-teal-500" />`;
                if (i % 10 == 0 && i != 0) {
